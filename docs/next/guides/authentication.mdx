...
Before you get started with implementing an authentication strategy, you'll need to setup two files, `src/pages/api/auth/wpe-headless.ts`, and `src/pages/api/auth/logout.ts`. Create the two files by copying the following code below:

```ts title=src/pages/api/auth/wpe-headless.ts
import 'faust.config';
...
import 'faust.config';
import { logoutHandler } from '@faustjs/core';
